...
/**
 * Janela de comparecimento (RF-014): só é possível marcar a partir de `janelaHoras`
...
/** Marca comparecimento de um aluno numa aula, respeitando janela, vaga e duplicidade (RF-013..016). */
export async function marcarComparecimento(params: {
  ...
  if (
    ...
    return { ok: false, motivo: "Fora da janela de comparecimento." }
  }

  // Reativa um comparecimento previamente cancelado, se existir (mantém histórico via @@unique).
  ...
  const comparecimento = await db.$transaction(async (tx) => {
    ...
    await criarNotificacao(tx, {
      ...
      tipo: "COMPARECIMENTO",
      titulo: novoStatus === "CONFIRMADO" ? "Comparecimento confirmado" : "Lista de espera",
      mensagem:
        novoStatus === "CONFIRMADO"
          ? `${aula.turma.nome ?? aula.turma.modalidade.nome}: comparecimento registrado.`
...
/** Cancela um comparecimento dentro do prazo configurado (RF-015/017). */
export async function cancelarComparecimento(params: {
  ...
  if (comparecimento?.status !== "CONFIRMADO" && comparecimento?.status !== "LISTA_ESPERA") {
    return { ok: false, motivo: "Sem comparecimento ativo." }
  }
...
/** Marca como NO_SHOW os comparecimentos confirmados sem check-in válido de uma aula encerrada (RF-018). */
  ...
  await db.$transaction(async (tx) => {
    for (const comparecimento of noShows) {
      ...
      await registrarLog(
        {
          ...
          justificativa: "Comparecimento sem check-in válido após encerramento da aula",
        },
        tx,
      )
      ...
      await criarNotificacao(tx, {
        ...
        mensagem: `${aula.turma.nome ?? aula.turma.modalidade.nome}: comparecimento sem check-in válido.`,
      })
    }
  })
...
async function promoverPrimeiroDaListaEspera(
  ...
  await registrarLog(
    {
      ...
      justificativa: "Vaga liberada por cancelamento de comparecimento confirmado",
    },
    tx,
  )
  ...
  await criarNotificacao(tx, {
    ...
    mensagem: `${proximo.aula.turma.nome ?? proximo.aula.turma.modalidade.nome}: seu comparecimento foi confirmado.`,
  })
}
