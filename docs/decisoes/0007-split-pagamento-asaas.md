...
- Nas cobranças de matrícula e de mensalidade, incluindo os ciclos futuros do PIX Automático, admitir uma
  contingência estreita quando o `POST /payments` com split retorna HTTP 400 `invalid_action`: repetir a
  busca por `externalReference`; se uma cobrança sem split já existir, recuperá-la sem repetir o POST;
  se a busca continuar vazia, marcar os splits ainda `PREPARADO` como `RECUSADO`, auditar a decisão e
  emitir a cobrança sem split. O recebimento permanece como obrigação de repasse manual.
...
- A única exceção é o split explicitamente recusado pela contingência acima, sem ID ou status remoto. Se a
  cobrança recuperada depois contiver qualquer split, ela volta a ser bloqueada para evitar repasse
  automático e manual em duplicidade.
