...
| Autocadastro com pré-seleção mensalista/Wellhub/TotalPass, declaração de benefício externo, plano padrão e primeira mensalidade Asaas somente para mensalista, comprovante privado opcional e aprovação administrativa (RF-001.1/001.2/RF-020/RF-046..048/RF-053.3/RF-079) | `Plano.padrao`,`SolicitacaoMatricula.tipoPagamento`,`SolicitacaoMatricula.beneficioAtivoDeclarado`,`CobrancaMatriculaAsaas`,`Usuario`,`Aluno`,`AlunoPlanoModalidade`,`Mensalidade`,`CobrancaAsaas`,`LogAuditoria` | `src/lib/services/matricula.service.ts`, `src/lib/services/pagamento-matricula.service.ts`, `src/lib/validations/matricula.ts`, `src/app/actions/matriculas.ts`, `src/app/(auth)/matricula`, `gestao/matriculas-pendentes`, `api/comprovantes-matricula/[id]`, `api/webhooks/asaas`, `checkin.service.ts` |
