...
modalidades, horários, comparecimentos, check-ins, presenças, horas treinadas, graduações, pagamentos e
...
Conceitos-chave (ver glossário): **comparecimento** (intenção) ≠ **check-in** (participação) ⇒ **presença**
...
grade para o aluno, comparecimento, check-in, presença automática por check-in válido, invalidação de
...
- **RF-003** Perfil do aluno (dados, tipo, status, modalidades, plano, situação financeira, históricos de
  comparecimento/check-in/invalidações/presença, horas gerais e por modalidade, graduações e histórico,
...
- **RF-009** Configurações da modalidade (nome, descrição, duração padrão, status, graduações associadas,
  professores habilitados, regras específicas de check-in/comparecimento).
...
### Comparecimento
- **RF-013** Marcação de comparecimento em aula disponível.
- **RF-014** Janela de comparecimento configurável (padrão 24h antes).
- **RF-015** Cancelamento de comparecimento (prazo configurável).
- **RF-016** Limite de vagas (capacidade; bloqueio ou lista de espera se ativa).
- **RF-017** Status do comparecimento (Confirmado, Cancelado pelo aluno, Cancelado pelo gestor, Convertido em
  check-in, Ausente, No-show).
- **RF-018** No-show: marcou comparecimento mas não fez check-in.
...
- **RF-020** Validação (aluno ativo; permissão; plano mensal interno adimplente se configurado; aula
  existente; há comparecimento se exigido; vaga disponível).
- **RF-021** Check-in associado ao comparecimento correspondente.
- **RF-022** Check-in sem comparecimento prévio: permitir / bloquear / apenas com aprovação.
...
- **RF-050** Adimplência. **RF-051** Bloqueio por inadimplência (alertar / bloquear comparecimento / bloquear check-in / não aplicar).
...
### Relatórios
- **RF-065..072** Alunos, comparecimentos, check-ins, presença, horas, graduação, financeiro, conciliação.

### Notificações
- **RF-073..078** Comparecimento, lembrete de treino, cancelamento de aula, financeiro, graduação, check-in invalidado (configurável).
...
RN-001 comparecimento não é presença · RN-002 check-in válido conta presença · RN-003 professor atua nas
...
CA-001 cadastro de aluno · CA-002 tipos válidos · CA-003 comparecimento na janela · CA-004 janela de 24h ·
...
Mensalista/Wellhub/TotalPass/Avulso seguem: cadastro → grade → comparecimento → check-in → presença → horas;
