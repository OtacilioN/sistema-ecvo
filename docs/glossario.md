...
Termos do domínio (pt-BR). Use exatamente esta terminologia em código, UI e relatórios.

| Termo | Significado |
| --- | --- |
| **Comparecimento** | Intenção do aluno de participar de um treino específico. **Não** gera presença nem horas (RN-001). Janela padrão: a partir de 24h antes da aula. |
...
| **Adimplência / Inadimplência** | Situação de pagamento de plano mensal interno (em dia / em atraso). Pode bloquear comparecimento/check-in conforme configuração (RF-051). |
...
| **No-show** | Aluno marcou comparecimento mas não fez check-in (RF-018). |
